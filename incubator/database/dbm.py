...
def check_if_short_posted(story, supabase_client=inject_supabase_client()) -> bool:
    hashed = generate_unique_id(story)
    data = supabase_client.table('videos_posted').select("*").eq('video_hash', hashed).execute()
    return len(data.data) > 0


def log_short_posted(title, story, supabase_client=inject_supabase_client()) -> bool:
    hashed = generate_unique_id(story)
    try:
        supabase_client.table('videos_posted').insert({
            "id": str(uuid.uuid4()),
